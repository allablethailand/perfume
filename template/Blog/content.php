    ...
    <?php foreach ($boxesNews as $index => $box): ?>
        <div class="box-news">
            <div class="box-image">
                ...
                <a href="?Blog_detail&id=<?php echo $encodedId; ?>&lang=<?php echo htmlspecialchars($lang); ?>" class="text-news">
                    ...
                    <?php elseif (!empty($box['image'])): ?>
                        <picture>
                            ...
                            <img src="<?= htmlspecialchars(str_replace('../','',$box['image'])) ?>" alt="Image for <?= htmlspecialchars($box['title']); ?>">
                        </picture>
                    <?php else: ?>
...
<div class="pagination">
    <?php 
    // กำหนดชื่อคีย์สำหรับหน้าหลัก ถ้าเป็นหน้า Blog ควรใช้ ?Blog
    $basePageParam = 'Blog'; 
    ?>
    
    <?php if ($page > 1): ?>
        <a href="?<?php echo $basePageParam; ?>&page=<?php echo $page - 1; ?>&search=<?php echo urlencode($searchQuery); ?>&lang=<?php echo htmlspecialchars($lang); ?>">
            <?= $lang === 'en' ? 'Previous' : ($lang === 'cn' ? '上一页' : ($lang === 'jp' ? '前へ' : ($lang === 'kr' ? '이전' : 'ก่อนหน้า'))); ?>
        </a>
    <?php endif; ?>

    <?php for ($i = 1; $i <= $totalPages; $i++): ?>
        <a href="?<?php echo $basePageParam; ?>&page=<?php echo $i; ?>&search=<?php echo urlencode($searchQuery); ?>&lang=<?php echo htmlspecialchars($lang); ?>" <?php echo $i == $page ? 'class="active"' : ''; ?>>
            <?php echo $i; ?>
        </a>
    <?php endfor; ?>
