...
try {
    ...
    if ($action == 'getData_orders') {
        ...
        $totalRecordsQuery = "SELECT COUNT(order_id) FROM orders WHERE del = 0";
        $totalRecords = $conn->query($totalRecordsQuery)->fetch_row()[0];
        
        $totalFilteredQuery = "SELECT COUNT(o.order_id) 
                              FROM orders o 
                              LEFT JOIN mb_user u ON o.user_id = u.user_id 
                              WHERE $whereClause";
        $totalFiltered = $conn->query($totalFilteredQuery)->fetch_row()[0];
        
        // เพิ่มการ JOIN กับ payment_slips เพื่อดึงข้อมูลสลิป
        $dataQuery = "SELECT 
                        o.*,
                        u.first_name,
                        u.last_name,
    ...
    // ========================================
        ...
        // Get order info with user details และ payment slip
    ...
    // ========================================
    // UPDATE ORDER STATUS (WITH AUTO STOCK DEDUCTION FOR confirmed)
    // ========================================
        ...
        try {
            ...
            // If status changed to 'confirmed' and payment is 'paid', deduct stock
            if ($new_status === 'confirmed' && $payment_status === 'paid' && $old_status !== 'confirmed') {
                ...
                while ($item = $items_result->fetch_assoc()) {
                    ...
                    logStockChange($conn, $product_id, 'deduct', $current_stock, $quantity, $new_stock, 
                        'order_confirmed', $order_id, $order_id, "Stock deducted - order status changed to confirmed", $created_by);
    ...
    // ========================================
        ...
        try {
            // Check if stock was deducted (order was confirmed)
            ...
            $was_confirmed = ($order_data['order_status'] === 'confirmed');
            $check_stmt->close();
            ...
            if ($was_confirmed) {
                ...
                while ($item = $items_result->fetch_assoc()) {
                    ...
                    if ($stock_result->num_rows === 0) {
                        continue; // Product might be deleted, skip
                    }
