    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .conversations-list {
            ...
            min-height: 0; padding-bottom: 150px;
        }
        ...
        .mode-3d-btn {
            ...
            box-shadow: 0 2px 8px rgba(102,126,234,0.3); margin-bottom: 10px;
        }
        .mode-3d-btn:hover { transform: translateY(-2px); box-shadow: 0 4px 16px rgba(102,126,234,0.5); }

        .edit-prompts-btn {
            width: 100%; padding: 12px 16px;
            background: linear-gradient(135deg, #1a1a1a 0%, #2d2d2d 100%);
            color: white; border: 1px solid #3a3a3a; border-radius: 8px;
            font-size: 13px; font-weight: 600; cursor: pointer;
            transition: all 0.3s ease;
            display: flex; align-items: center; justify-content: center; gap: 10px;
        ...
        .edit-prompts-btn:hover::before { left: 100%; }
        .edit-prompts-btn:hover { transform: translateY(-2px); box-shadow: 0 4px 16px rgba(0,0,0,0.25); }

        /* ========== Chat Main ========== */
        .chat-main {
            display: flex; flex-direction: column;
            background: #fafafa; height: 100%; overflow: hidden;
        }
        ...
        .ai-avatar { width: 40px; height: 40px; border-radius: 50%; object-fit: cover; }
        .ai-info h3 { font-size: 16px; font-weight: 600; margin-bottom: 2px; }
        .ai-info p  { font-size: 12px; color: #666; }

        /* ========== Messages ========== */
        .chat-messages {
            flex: 1; overflow-y: auto !important; overflow-x: hidden;
            padding: 30px; min-height: 0; scroll-behavior: smooth;
        }
        ...
        .empty-state {
            ...
            justify-content: center; height: 100%; color: #999; text-align: center;
        }
        ...
        .message-avatar-wrap {
            ...
            flex-shrink: 0;
            width: 36px;
            align-self: flex-end; /* ชิดล่าง ให้ popup ลอยขึ้นข้างบน */
        }
        .message.user .message-avatar-wrap { order: 2; }
        ...
        /* ✅ message-content — position relative เพื่อให้ popup absolute ได้ */
        .message-content {
            background: #fff; padding: 12px 16px; border-radius: 12px;
            box-shadow: 0 1px 2px rgba(0,0,0,0.05); max-width: 70%;
            position: relative; /* ✅ สำคัญ — popup อ้างอิง position นี้ */
            overflow: visible;  /* ✅ ให้ popup ลอยออกนอกได้ */
        }
        ...
        /* ✅ Emotion popup — ลอยเหนือ message-content */
        .emotion-popup {
            position: absolute;
            bottom: calc(100% + 8px); /* เหนือขอบบนของ bubble 8px */
            left: 0;
            width: 120px;
            height: 160px;
            border-radius: 12px;
        ...
        @keyframes popupOut {
            ...
            to   { opacity: 0; transform: scale(0.75) translateY(12px);  }
        }

        /* Typing */
        .typing-indicator { display: none; padding: 0 30px 20px; }
        .typing-indicator.active { display: flex; gap: 12px; animation: fadeIn 0.3s ease-in; }
        ...
        .chat-input {
            ...
            border-top: 1px solid #e0e0e0; flex-shrink: 0; min-height: 80px;
        }
        .input-wrapper {
            display: flex; gap: 12px; max-width: 900px;
            margin: 0 auto; align-items: flex-end;
        }
        .message-input {
            flex: 1; border: 1px solid #e0e0e0; border-radius: 24px;
            padding: 12px 20px; font-size: 14px; resize: none;
            max-height: 120px; min-height: 44px; font-family: inherit; line-height: 1.5;
        }
        .message-input:focus { outline: none; border-color: #000; }
        .send-btn {
        ...
        .conversation-item.active { background: #000; color: #fff; border-color: #000; }
        ...
        .delete-conv-btn {
            ...
            padding: 4px 8px; cursor: pointer; opacity: 0; transition: all 0.2s; color: #dc3545;
        }
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('switch3DBtn').addEventListener('click', function() {
            let url = '?ai_chat_3d&lang=' + lang;
            if (aiCode) url += '&ai_code=' + aiCode;
            window.location.href = url;
        });

        document.getElementById('editPromptsBtn').addEventListener('click', function() {
